 ...
 */
function resolveIntent(userIntent, tokenAgeSec) {
    if (userIntent)
 ...
 */
function resolveTheme(userTheme, utilityClass) {
    if (userTheme)
        return userTheme.toLowerCase();
    switch (utilityClass) {
        case "protocol":
            return "minimalist";
        case "culture":
 ...
 */
function resolveProfileName(utilityClass, intent, theme) {
 ...
 */
export function resolveToneProfile(utilityClass, userIntent, userTheme, tokenAgeSec) {
    ...
    log.info({ intent, theme, profileName, autoIntent: !userIntent, autoTheme: !userTheme }, `Tone Profile resolved: "${profileName}"`);
    return profile;
}
//# sourceMappingURL=toneProfile.js.map