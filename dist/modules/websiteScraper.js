...
export async function scrapeWebsite(websiteUrl) {
    ...
    try {
        const html = await withRetry(() => fetchPage(websiteUrl), { label: "website-scrape", maxRetries: 2 });
        const extractedText = extractContent(html);
        if (!extractedText || extractedText.length < 20) {
            log.warn({ websiteUrl }, "Insufficient content extracted");
            return { found: false };
        }
        log.info({ websiteUrl, length: extractedText.length }, "Website content extracted");
...
async function fetchPage(url) {
    const res = await axios.get(url, {
        ...
        maxRedirects: 3,
        headers: {
            "User-Agent": "Mozilla/5.0 (compatible; HypepackBot/1.0; +https://hypepack.io)",
            Accept: "text/html",
        },
        responseType: "text",
    });
...
//# sourceMappingURL=websiteScraper.js.map