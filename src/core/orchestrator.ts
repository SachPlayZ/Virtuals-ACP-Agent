...
export async function runPipeline(
    ...
    log.info(
        { projectName: brief.projectName, utilityClass: brief.utilityClass, oneLiner: brief.oneLiner },
        "✅ Step 1.5: Creative Brief built"
    );
    ...
    log.info(
        { intent: tone.intent, theme: tone.theme, profileName: tone.profileName },
        `✅ Stage 2: Tone Profile resolved → "${tone.profileName}"`
    );
    ...
    log.info(
        { elapsed: `${stageTimer3.elapsed().toFixed(1)}s` },
        "✅ Stage 3 complete"
    );
...
function extractCta(firstPost: string, ticker: string): string {
