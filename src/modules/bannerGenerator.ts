...
import type { BannerResult, VisualThemes, LogoResult } from "../types/job.types.js";
 ...
 * 1. Generate background via OpenAI DALL-E 3
...
export async function generateBanner(
    ...
    try {
        bgUrl = await withRetry(
            () => generateBackground(visualThemes.image_prompt),
...
async function generateBackground(prompt: string): Promise<string> {
    ...
    const response = await client.images.generate({
        model: "dall-e-3",
        prompt: `${prompt}, ultra high resolution, cinematic, no text, no typography, no letters, no words`,
        n: 1,
        size: "1792x1024",
        quality: "hd",
    });
