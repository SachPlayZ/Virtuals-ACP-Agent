import type { UtilityClass } from "../types/job.types.js";
...
export function classifyUtility(
    ...
    for (const s of PROTOCOL_SIGNALS) {
        if (text.includes(s)) protocolHits++;
    }
    for (const s of CULTURE_SIGNALS) {
        if (text.includes(s)) cultureHits++;
    }

    let result: UtilityClass;
    ...
    log.info(
        { protocolHits, cultureHits, result },
        `Utility classified as "${result}"`
    );
    return result;
}
